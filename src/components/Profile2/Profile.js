...
function Profile() {
    ...
    useEffect(()=>{
       Firebase.firestore().collection('users').where("email",'==',data.state.id).get().then((snapshot)=>{
        snapshot.forEach(function(doc) {
          ...
          setUser(doc.data())
            ...
            Firebase.firestore().collection('users').doc(id1).collection('feedback').onSnapshot((snapshot)=>{
                console.log('hii')
